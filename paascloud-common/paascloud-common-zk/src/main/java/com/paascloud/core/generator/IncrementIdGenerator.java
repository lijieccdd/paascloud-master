...
public class IncrementIdGenerator implements IdGenerator {
	...
	public Long nextId() {
		...
		if (regCenter.isExisted(path)) {
			...
			String exisitId = regCenter.getDirectly(GlobalConstant.ZK_REGISTRY_ID_ROOT_PATH + GlobalConstant.Symbol.SLASH + app + GlobalConstant.Symbol.SLASH + host);
			return Long.valueOf(exisitId);
		} else {
